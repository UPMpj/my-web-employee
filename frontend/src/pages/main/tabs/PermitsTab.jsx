...
export default function PermitsTab({ empId }) {
  ...
  const submit = async (e) => {
    e.preventDefault();
    if (!form.permit_type) { toast.error(t("err_permit_type")); return; }
    setSaving(true);
    try {
  ...
  return (
    <div>
      ...
      <div className="ed-tab-topbar">
        <h2 className="ed-section-title" style={{margin:0}}>{t("permit_title")}</h2>
        <button className="ed-add-btn" onClick={openNew}>{t("permit_add_btn")}</button>
      </div>
        ...
        <form className="ed-doc-form" onSubmit={submit}>
          <div className="ed-form-row">
            <div className="ed-form-group">
              <label className="ed-form-label">{t("permit_type_lbl")}</label>
            ...
            <div className="ed-form-group" style={{flex:2}}>
              <label className="ed-form-label">{t("permit_number_lbl")}</label>
              <input className="ed-form-input" value={form.permit_number} onChange={e => setForm(f => ({...f, permit_number:e.target.value}))} placeholder={t("permit_ph_number")} />
            </div>
            <div className="ed-form-group">
              <label className="ed-form-label">{t("permit_status_lbl")}</label>
          ...
          <div className="ed-form-row">
            ...
            <div className="ed-form-group" style={{flex:2}}>
              ...
              <input className="ed-form-input" value={form.notes} onChange={e => setForm(f => ({...f, notes:e.target.value}))} placeholder={t("permit_ph_notes")} />
            </div>
          </div>
          <div className="ed-form-row">
            <div className="ed-form-group" style={{flex:1}}>
              <label className="ed-form-label">{t("permit_file_lbl")}</label>
          ...
          <div style={{display:"flex",gap:8,justifyContent:"flex-end"}}>
            ...
            <button type="submit" className="ed-save-btn" disabled={saving}>{saving ? t("saving") : editing ? t("permit_edit_save") : t("save")}</button>
          </div>
        </form>
      )}

      {loading ? <div className="ed-empty-tab">{t("loading")}</div> : permits.length === 0 ? (
        <div className="ed-empty-tab">{t("permit_no_data")}</div>
      ) : (
        <table className="ed-info-table" style={{marginTop:16}}>
          <thead>
            <tr>
              <th className="ed-th">{t("permit_photo_col")}</th><th className="ed-th">{t("doc_type_lbl")}</th><th className="ed-th">{t("permit_no_col")}</th>
              <th className="ed-th">{t("permit_issued_col")}</th><th className="ed-th">{t("permit_expiry_col")}</th>
              <th className="ed-th">{t("status")}</th><th className="ed-th">{t("notes")}</th><th className="ed-th"></th>
            </tr>
          </thead>
          <tbody>
            {permits.map(p => {
              ...
              return (
                <tr key={p.permit_id}>
                  ...
                  <td className="ed-val" style={{display:"flex",gap:6}}>
                    <button className="ed-edit-mini-btn" onClick={() => openEdit(p)}>{t("permit_edit_btn")}</button>
                    <button className="ed-del-btn" onClick={() => del(p.permit_id)}>{t("permit_del_btn")}</button>
                  </td>
                </tr>
              );
            })}
