...
export default function ImportEmployee() {
  ...
  /* parse file on frontend for preview */
  const handleFile = async (e) => {
    ...
    try {
      ...
      setRows(r.data.rows);
      setStep(2);
      toast.success(`ອ່ານໄດ້ ${r.data.total} ແຖວ`);
    } catch (err) {
  ...
  const handleCommit = async () => {
    ...
    try {
      ...
      setResult(r.data);
      setStep(3);
      toast.success(`ນຳເຂົ້າສຳເລັດ ${r.data.inserted} ຄົນ`);
    } catch {
      toast.error("ນຳເຂົ້າຜິດພາດ");
    }
    setLoading(false);
  };

  const reset = () => { setRows([]); setResult(null); setStep(1); };
    ...
    <div className="imp-page">
      <div className="imp-header">
        <div>
          ...
          <p className="imp-sub">ອັບໂຫລດໄຟລ໌ Excel ຫລື CSV ເພື່ອນຳເຂົ້າພະນັກງານຫຼາຍຄົນພ້ອມກັນ</p>
        </div>
      </div>
        ...
        <div className="imp-card">
          ...
          <div className="imp-template-row">
            <div className="imp-template-info">
              ...
              <div>
                <div className="imp-template-label">ດາວໂຫລດ Template</div>
                <div className="imp-template-sub">ໄຟລ໌ Excel ທີ່ມີ column ຖືກຕ້ອງ ພ້ອມຕົວຢ່າງ</div>
              </div>
            </div>
          ...
          <div className="imp-drop-zone" onClick={() => fileRef.current.click()}>
            {loading ? (
              <div className="imp-drop-content">ກຳລັງອ່ານໄຟລ໌...</div>
            ) : (
              <div className="imp-drop-content">
                <div className="imp-drop-icon">📂</div>
                <div className="imp-drop-label">ກົດເພື່ອເລືອກໄຟລ໌ Excel (.xlsx) ຫລື CSV (.csv)</div>
                <div className="imp-drop-sub">ຂະໜາດສູງສຸດ 10MB — ຮອງຮັບສູງສຸດ 200 ແຖວ</div>
              </div>
            )}
          ...
          <div className="imp-preview-header">
            <div>
              ...
              <div className="imp-preview-stats">
                <span className="imp-stat-ok">✓ {validRows.length} ແຖວ ຖືກຕ້ອງ</span>
                {invalidRows.length > 0 && <span className="imp-stat-err">✗ {invalidRows.length} ແຖວ ຜິດ</span>}
              </div>
            </div>
            <div style={{display:"flex", gap:8}}>
              <button className="imp-back-btn" onClick={reset}>‹ ກັບ</button>
              <select className="imp-select" style={{width:"auto"}} value={company} onChange={e => setCompany(e.target.value)}>
                {companies.map(c => (
                  <option key={c.company_id} value={c.company_id}>{c.companies_name}</option>
                ))}
              </select>
              <button className="imp-commit-btn" onClick={handleCommit} disabled={loading || validRows.length === 0}>
                {loading ? "ກຳລັງນຳເຂົ້າ..." : `ນຳເຂົ້າ ${validRows.length} ຄົນ`}
              </button>
            </div>
          </div>
            ...
            <table className="imp-table">
              <thead>
                <tr>
                  ...
                  <th className="imp-th">Employee Code</th>
        ...
        <div className="imp-card imp-done-card">
          ...
          <div className="imp-done-stats">
            ...
            <div className="imp-done-stat">
              ...
              <div className="imp-done-lbl">ຂ້າມ (ຜິດ/ຊໍ້າ)</div>
            </div>
          </div>
          ...
          <div style={{display:"flex", gap:12, justifyContent:"center", marginTop:24}}>
            <button className="imp-commit-btn" onClick={reset}>ນຳເຂົ້າໃໝ່</button>
